...
Context: Flutter/Dart mobile app for Field Service Management. Clean Architecture with feature-first modules, heavy code generation, type-safe routing, DI, offline-first caching. See `CLAUDE.md` for deep background; this file is your quick operator manual.
...
- Routing via Auto Route v9. Pages use `@RoutePage()`. Router config in `lib/core/router/app_router.dart` with generated `app_router.gr.dart`.
- Networking: `Dio` configured in `core/network/dio_client.dart` with `AuthInterceptor` (JWT attach, 401 refresh). APIs via Retrofit interfaces in feature `data/api/*`.
...
- Convenience scripts: `scripts/run_*.sh` and `scripts/build_*.sh` use flavors and targets.
  ...
  - Run staging: `flutter run --flavor staging --target lib/main_staging.dart`
  - Run prod: `flutter run --flavor prod --target lib/main_prod.dart --release`
