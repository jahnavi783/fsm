...
class WorkOrderForms {
  ...
  /// Build form for completing a work order - Step 1: Customer Info + Work Log
  /// Fields: customerName (required), workLog (required)
  ...
  /// Build form for completing a work order - Step 2: Parts Used
  /// Fields: parts (FormArray - optional but if added, quantity required)
  ...
  /// Build form for completing a work order - Step 3: Work Images
  ...
  /// Build form for completing a work order - Step 4: Signature + Notes
  /// Fields: signature (required), completionNotes (optional)
  static FormGroup buildCompleteFormStep4() {
    return FormGroup({
  ...
  static Map<String, dynamic> getCompleteFormData(
    ...
    final partsArray =
        step2.control('parts') as FormArray<Map<String, dynamic>>;
    ...
    return {
      'customerName':
          step1.control('customerName').value?.toString().trim() ?? '',
      'workLog': step1.control('workLog').value?.toString().trim() ?? '',
      'parts': parts,
      'files': (step3.control('files').value as List<File>?) ?? [],
      'signature': step4.control('signature').value as File?,
      'completionNotes':
          step4.control('completionNotes').value?.toString().trim(),
    };
  }
}
