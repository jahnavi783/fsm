...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Widget _buildListContent(WorkOrdersListState state, dynamic user) {
    return Column(
      children: [
        ...
        state.maybeWhen(
          loaded: (workOrders, _, __, ___, ____, _____, ______, _______, ________, _________, __________) {
            final inProgressWO = workOrders.where((wo) => 
                wo.status == WorkOrderStatus.inProgress).firstOrNull;
            if (inProgressWO != null) {
              return Padding(
                padding: const EdgeInsets.all(16.0),
                child: CurrentWorkOrderCard(
                  workOrder: inProgressWO,
                  onTap: () => _handleWorkOrderAction(context, inProgressWO, 'view'),
                  onPause: () => _handleWorkOrderAction(context, inProgressWO, 'pause'),
                  onParts: () => _handleWorkOrderAction(context, inProgressWO, 'parts'),
                  onDocs: () => _handleWorkOrderAction(context, inProgressWO, 'docs'),
                  onComplete: () => _handleWorkOrderAction(context, inProgressWO, 'complete'),
                ),
              );
            }
        ...
        Expanded(
          child: state.maybeWhen(
            loaded: (workOrders, _, __, ___, ____, isLoadingMore, ______, _______, ________, _________, __________) {
              final filteredWorkOrders = _getFilteredWorkOrders(workOrders, _selectedFilter);
                ...
                itemBuilder: (context, index) {
                  ...
                  return Padding(
                    ...
                    child: WorkOrderListCard(
                      workOrder: workOrder,
                      onTap: () => _handleWorkOrderAction(context, workOrder, 'view'),
                      onStart: _selectedFilter == 'assigned' || _selectedFilter == 'created' 
  ...
  List<WorkOrderEntity> _getFilteredWorkOrders(List<WorkOrderEntity> workOrders, String filter) {
    switch (filter) {
      case 'all':
        return workOrders;
      case 'unassigned':
        return workOrders.where((wo) => wo.status == WorkOrderStatus.created).toList();
      case 'assigned':
        return workOrders.where((wo) => wo.status == WorkOrderStatus.assigned).toList();
      case 'in_progress':
        return workOrders.where((wo) => wo.status == WorkOrderStatus.inProgress).toList();
      case 'paused':
        return workOrders.where((wo) => wo.status == WorkOrderStatus.paused).toList();
      case 'completed':
        return workOrders.where((wo) => wo.status == WorkOrderStatus.completed).toList();
      default:
        return workOrders;
    }
  }
  ...
  List<StatsCardData> _getStatsData(WorkOrdersListState state) {
    return state.maybeWhen(
      loaded: (workOrders, unassignedWorkOrders, unassignedCount, _, __, ___, ____, _____, ______, _______, __________) {
        return [
          StatsCardData(
            title: 'Assigned',
            count: workOrders.where((wo) => wo.status == WorkOrderStatus.assigned).length.toString(),
          ...
          StatsCardData(
            title: 'In Progress',
            count: workOrders.where((wo) => wo.status == WorkOrderStatus.inProgress).length.toString(),
          ...
          StatsCardData(
            title: 'Paused',
            count: workOrders.where((wo) => wo.status == WorkOrderStatus.paused).length.toString(),
          ...
          StatsCardData(
            title: 'Completed',
            count: workOrders.where((wo) => wo.status == WorkOrderStatus.completed).length.toString(),
